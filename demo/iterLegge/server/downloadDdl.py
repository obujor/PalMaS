...
{
    ?ddl a osr:Ddl; osr:legislatura %s; osr:fase "%s"^^<http://www.w3.org/2001/XMLSchema#string>; osr:idDdl ?idddl.
}""" % (LEGISLATURA, FASE,)
...
if len(results["results"]["bindings"]) > 0 :
    idDdl = int(results["results"]["bindings"][0]['idddl']['value'])
else:
    exit()
...
{
    ?ddl a osr:Ddl; osr:legislatura %s; osr:statoDdl ?stato; osr:dataStatoDdl ?dataStato; osr:dataPresentazione ?dataPresentazione;
